...
export type MutationEvent =
  | {
      type: "childList";
      parentId: number;
      adds: { node: SerializedNode; nextSiblingId: number | null }[];
      removeIds: number[];
    }
  | { type: "attributes"; targetId: number; name: string; value: string | null }
  | { type: "characterData"; targetId: number; value: string };
...
export function startObserver(
  doc: Document,
  onMutations: (events: MutationEvent[]) => void,
): () => void {
  const observer = new MutationObserver((mutations: MutationRecord[]) => {
    ...
    for (const mutation of mutations) {
      if (mutation.type === "childList") {
        ...
        const value = (mutation.target as Element).getAttribute(name);
