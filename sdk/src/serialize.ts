...
export interface SerializedNode {
  id: number;
  type: number; // Node.ELEMENT_NODE = 1, TEXT_NODE = 3, etc.
  tagName?: string;
...
function isMasked(node: Node): boolean {
  ...
  while (current) {
    if (
      ...
      (current as Element).hasAttribute("data-spectator-mask")
    ) {
      return true;
    }
...
export function serializeNode(node: Node): SerializedNode | null {
  ...
  if (node.nodeType === Node.DOCUMENT_NODE) {
    return serializeNode((node as Document).documentElement);
  }
  ...
  if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    if (isMasked(el) && (tag === "input" || tag === "textarea")) {
      ...
      if (val) attributes["value"] = "•".repeat(val.length);
    }
 ...
 */
export function takeFullSnapshot(doc: Document): SerializedNode | null {
  nextId = 1;
  nodeIdMap.clear();
 ...
 */
export function lookupNodeId(node: Node): number | undefined {
  return nodeIdMap.get(node);
}
